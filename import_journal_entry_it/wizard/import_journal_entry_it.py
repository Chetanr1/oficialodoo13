...
class ImportJournalEntryIt(models.TransientModel):
	...
	document_file = fields.Binary(string='Excel', help="El archivo Excel debe ir con la cabecera: date, ref, glosa, journal_id, invoice_date, tc, currency_id, type_document_id, partner_id, account_id, debit, credit, amount_currency, nro_comp, date_maturity, name, analytic_account_id, tag_ids, amount_tax")
			...
			if row_no <= 0:
				...
				if len(line) == 23:
					...
					if line[4] != '':
						a1 = int(float(line[4]))
						a1_as_datetime = datetime(*xlrd.xldate_as_tuple(a1, workbook.datemode))
						invoice_date_string = a1_as_datetime.date().strftime('%Y-%m-%d')
					
					if line[18] != '':
						a1 = int(float(line[18]))
						a1_as_datetime = datetime(*xlrd.xldate_as_tuple(a1, workbook.datemode))
						date_maturity_string = a1_as_datetime.date().strftime('%Y-%m-%d')
					values = ({'number':line[0],
								...
								'debit':line[13],
								'credit':line[14],
								'currency_id_aml':line[15] if line[15] else 'PEN',
								'amount_currency':line[16],
								'nro_comp':line[17],
								'date_maturity':date_maturity_string,
								'name':line[19],
								'analytic_account_id':line[20],
								'tag_ids':line[21],
								'amount_tax':line[22],
