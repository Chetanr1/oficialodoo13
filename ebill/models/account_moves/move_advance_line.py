...
class MoveAdvanceLine(models.Model):
    ...
    account_line_move_id = fields.Many2one('account.move.line',string="Producto",
                                           required=True,domain="[('move_id', '=', move_id ),('is_anticipo','!=',False),('product_id','!=',False)]")
    anticipo_id = fields.Many2one('account.move', string="Anticipo")
    account_line_move_id_name = fields.Char(compute="get_name_anticipo",string='Anticipo')
    @api.depends('account_line_move_id_name')
    ...
    _sql_constraints = [
        ('unique_anticipo_id', 'unique(anticipo_id)', 'ya se ingreso ese anticipo')
    ]
