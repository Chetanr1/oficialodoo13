...
class AccountTax(models.Model):
	...
	def compute_all_origin(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False,
		...
		for tax in self.filtered(lambda r: r.amount_type == 'code'):
			...
			if localdict.get('result', False):
				taxes += tax
		return self.compute_all_origin_origin(price_unit, currency, quantity, product, partner,
														  is_refund=is_refund,
														  handle_price_include=handle_price_include)

	def compute_all_origin_origin(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False,
					handle_price_include=True):
		...
		for tax in taxes:
			...
			if base_excluded_flag and included_flag:
				raise UserError(('Unable to mix any taxes being price included with taxes affecting the base amount but not included in price.'))
		...
		for tax in taxes:
			...
			repartition_line_amounts = [round(tax_amount * line.factor, prec) for line in
										tax_repartition_lines]
			total_rounding_error = round(factorized_tax_amount - sum(repartition_line_amounts), prec)
			#nber_rounding_steps = int(abs(total_rounding_error / currency.rounding))
			nber_rounding_steps = int(abs(total_rounding_error ))
		...
		return {
			...
			'taxes': taxes_vals,
			'total_excluded': round(sign * total_excluded,10),
			#'total_included': sign * currency.round(total_included),
			'total_included': round(sign *total_included,10),
			#'total_void': sign * currency.round(total_void),
			'total_void': round(sign * total_void,10),
		}
